...
def get_phreeqc_data():
    ...
    """
    with open("phreeqc_data/phreeqc_data.json") as json_data:
...
def plot_data_sets(xdata, phreeqc_data, reaktoro_output_dict, data_type, xlabel):
    fig, ax = plt.subplots()
    fig.set_dpi(300)
...
def plot_result(
    xdata, reaktoro_data, phreeqc_data, xlabel=None, ylabel=None, sweep_name=None
):
    fig, ax = plt.subplots()
    fig.set_dpi(300)
    fig.set_size_inches(3.25, 3.25, forward=True)
