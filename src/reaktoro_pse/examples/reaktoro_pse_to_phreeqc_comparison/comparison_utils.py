...
def get_phreeqc_data(data_type="phreeqc_data.json"):
    ...
    path = os.path.join(
        path_rktpse,
        f"examples/reaktoro_pse_to_phreeqc_comparison/{data_type}",
    )
    ...
    for i, key in enumerate(
        ...
        if key in reaktoro_output_dict[data_type]:
            if key == "pE":
                ...
                phreeqc_key = key
            if phreeqc_key in phreeqc_data[data_type]:
                reaktor_data_array = np.array(reaktoro_output_dict[data_type][key])
                phreeqc_data_array = np.array(phreeqc_data[data_type][phreeqc_key])
                percent_dif = (
